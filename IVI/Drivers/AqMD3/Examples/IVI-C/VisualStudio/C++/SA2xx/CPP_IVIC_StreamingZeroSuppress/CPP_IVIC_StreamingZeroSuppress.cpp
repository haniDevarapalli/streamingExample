...
int main()
{
    ...
    try
    {
        ...
        checkApiCall( AqMD3_SetAttributeViInt32( session, "", AQMD3_ATTR_ACQUISITION_MODE, AQMD3_VAL_ACQUISITION_MODE_NORMAL) );
        ...
        while( system_clock::now() < endTime )
        {
            ...
            for (auto const& record : recordDescriptorList)
            {
                ...
                if (LibTool::MarkerTag::TriggerNormal != record.GetTriggerMarker().tag)
                    throw std::runtime_error("Unexpected trigger tag, got " + ToString(int(record.GetTriggerMarker().tag)) + ", expected " + ToString(int(LibTool::MarkerTag::TriggerNormal)));
